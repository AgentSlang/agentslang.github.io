 ...
 * {@link org.agent.slang.dm.narrative.data.StateChangeData state changes} (to
...
@ConfigureParams(mandatoryConfigurationParams = "modelPath",
                 inputDataTypes = StateChangeData.class,
                 outputChannels = "stateChange.data", outputDataTypes = StateChangeData.class)
public class StoryGraphComponent extends MixedComponent {
    ...
    public StoryGraphComponent(String outboundPort, ComponentConfig config) {
    ...
    protected void setupComponent(ComponentConfig config) {
        gui = new StoryGraphFrame(config.getFileProperty(MODEL_PROP, true), "story/Story.xml", "steps", "Step_(?<i>\\d+)\\.xml");
    ...
    protected void handleData(GenericData data) {
        if (data instanceof StateChangeData) {
            gui.setStateNumber(((StateChangeData) data).getNewStateNumber());
        }
    }
